...
jewel.board = (function () {
    ...
    function initialize(startJewels, callback) {
        ...
        numJewelType = settings.numJewelTypes;
        baseScore = settings.baseScore;
        cols = settings.cols;
        rows = settings.rows;
        if(startJewels){
            jewelsBoard = startJewels;
        } else {
            fillBoard();
        }
        callback && callback();
    ...
    function randomJewel() {
        return Math.floor(Math.random() * numJewelType);
    }
    ...
    function canSwap(x1, y1, x2, y2) {
        ...
        jewelsBoard[x1][y1] = type2;
        jewelsBoard[x2][y2] = type1;

        chain = ( checkChain(x2, y2) > 2 || checkChain(x1, y1) > 2 );

        //swap back
        jewelsBoard[x1][y1] = type1;
        jewelsBoard[x2][y2] = type2;
